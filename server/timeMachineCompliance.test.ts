...
// HARVEST NOTE (base consolidation): two `it.skip` cases below assert exact
// year-by-year figures from shared/indexCreditingData.ts. Their expected values
// were written against the 688 build's copy of that file, whose figures match no
// S&P calendar return (e.g. 2009: 50.25, 2008: -44.76). The trunk's copy is the
// sourced one — its header cites the S&P price-return table read 14 Sep 2026
// (2008: -38.3, 2009: 23.5). The tests are wrong, not the data. Re-derive the two
// expected values from the trunk series and un-skip; the Shiller price-return
// adapter is the longer-term source of truth for this file.
...
describe("alternate-age back-test — real years, paired with the buyer's reality", () => {
  ...
  it.skip("runs the actual index years — nothing scaled, mapped or derived", () => {
...
describe("archaeology — allocations over windows, as history not projection", () => {
  ...
  it.skip("runs one allocation on real index years", () => {
