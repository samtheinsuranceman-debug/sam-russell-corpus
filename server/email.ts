...
<head>
  ...
  <style>
    body { margin: 0; padding: 0; background: #F7F4EE; font-family: -apple-system, BlinkMacSystemFont, 'IBM Plex Sans', 'Inter', sans-serif; color: #1C2230; }
    .container { max-width: 560px; margin: 40px auto; padding: 0 20px; }
    .card { background: #FFFFFF; border: 1px solid #D9D4C8; border-radius: 16px; padding: 40px; }
    .logo { font-size: 22px; font-weight: 800; color: #0B1220; letter-spacing: -0.5px; margin-bottom: 32px; }
    .logo span { color: #0B1220; }
    h1 { font-size: 24px; font-weight: 700; color: #0B1220; margin: 0 0 12px; }
    p { font-size: 15px; line-height: 1.6; color: #4A5568; margin: 0 0 16px; }
    .role-badge { display: inline-block; background: #0B12201A; border: 1px solid #0B122033; color: #0B1220; font-size: 12px; font-weight: 600; padding: 4px 12px; border-radius: 20px; margin-bottom: 24px; }
    .cta { display: block; background: #0B1220; color: #F7F4EE !important; text-decoration: none; font-weight: 700; font-size: 15px; text-align: center; padding: 14px 28px; border-radius: 10px; margin: 28px 0; }
    .cta:hover { background: #0B1220; }
    .url-box { background: #F7F4EE; border: 1px solid #D9D4C8; border-radius: 8px; padding: 12px 16px; font-size: 12px; color: #4A5568; word-break: break-all; margin-bottom: 24px; }
    .footer { font-size: 12px; color: #6A7383; margin-top: 32px; text-align: center; }
    .divider { border: none; border-top: 1px solid #D9D4C8; margin: 24px 0; }
  </style>
</head>
<body>
  <div class="container">
    <div class="card">
      ...
      <p><strong style="color:#0B1220">${opts.inviterName}</strong> has invited you to join <strong style="color:#0B1220">${opts.workspaceName}</strong> on Russell Capital Systems™.</p>
      <div class="role-badge">${roleLabel}</div>
      <p>Click the button below to accept your invitation and set up your account. This link expires on <strong style="color:#1C2230">${expiryStr}</strong>.</p>
    ...
    <div class="footer">
      ...
      <a href="mailto:support@russellcapitalsystems.com" style="color:#6A7383">support@russellcapitalsystems.com</a>
    </div>
  </div>
</body>
</html>`;
...
<body style="margin:0;padding:0;background:#F7F4EE;font-family:-apple-system,BlinkMacSystemFont,'Inter',sans-serif;color:#1C2230;">
  <div style="max-width:560px;margin:0 auto;padding:32px 24px;">
    <h1 style="color:#3D6B4F;font-size:20px;margin:0 0 16px;">Thanks, ${name} — we've got it.</h1>
    ...
    <p style="line-height:1.6;color:#6A7383;font-size:13px;border-top:1px solid #D9D4C8;padding-top:16px;margin-top:24px;">Any strategies or figures you saw are general education only — not tax, legal, or investment advice, and not guaranteed. Every result is reviewed by our tax professional team for suitability and compliance with applicable IRS statutes before anything is implemented, and your own results may differ.</p>
...
<body style="margin:0;padding:0;background:#F7F4EE;font-family:-apple-system,BlinkMacSystemFont,'Inter',sans-serif;color:#1C2230;">
  <div style="max-width:560px;margin:0 auto;padding:32px 24px;">
    <h1 style="color:#3D6B4F;font-size:20px;margin:0 0 16px;">New homepage lead: ${esc(opts.who)}</h1>
    ...
    <p style="line-height:1.6;"><a href="${esc(opts.inboxUrl ?? "/portal/leads")}" style="color:#3D6B4F;">Open the lead inbox</a> to review the full fact-finder and the advisor figures.</p>
  </div>
</body></html>`;
  const result = await sendMail({ to: opts.toEmail, subject, text, html: sanitizeEmailHtml(html) });
...
export async function sendStaleClientDigest(opts: StaleDigestOptions): Promise<{ sent: boolean; reason?: string; clientCount: number }> {
  ...
  const clientRows = opts.staleClients.slice(0, 25).map(c => {
    ...
    return `<tr>
      <td style="padding:10px 12px;border-bottom:1px solid #D9D4C8;color:#0B1220;font-weight:500">${c.name}</td>
      <td style="padding:10px 12px;border-bottom:1px solid #D9D4C8;color:#4A5568">${c.email ?? "—"}</td>
      <td style="padding:10px 12px;border-bottom:1px solid #D9D4C8;color:#4A5568">${lastDate}</td>
      <td style="padding:10px 12px;border-bottom:1px solid #D9D4C8;text-align:center"><span style="background:${c.daysSinceContact >= 60 ? '#A3322D1A;color:#A3322D' : '#8A6A1F1A;color:#8A6A1F'};font-size:12px;font-weight:600;padding:3px 10px;border-radius:12px">${c.daysSinceContact}d</span></td>
    </tr>`;
  }).join("");
...
<head>
  ...
  <style>
    body { margin:0; padding:0; background:#F7F4EE; font-family:-apple-system,BlinkMacSystemFont,'Inter',sans-serif; color:#1C2230; }
    .container { max-width:640px; margin:40px auto; padding:0 20px; }
    .card { background:#FFFFFF; border:1px solid #D9D4C8; border-radius:16px; padding:40px; }
    .logo { font-size:22px; font-weight:800; color:#0B1220; letter-spacing:-0.5px; margin-bottom:32px; }
    .logo span { color:#0B1220; }
    h1 { font-size:22px; font-weight:700; color:#0B1220; margin:0 0 8px; }
    p { font-size:15px; line-height:1.6; color:#4A5568; margin:0 0 16px; }
    .alert-badge { display:inline-block; background:#8A6A1F1A; border:1px solid #8A6A1F33; color:#8A6A1F; font-size:13px; font-weight:600; padding:4px 14px; border-radius:20px; margin-bottom:20px; }
    table { width:100%; border-collapse:collapse; margin:20px 0; }
    th { text-align:left; padding:8px 12px; font-size:12px; font-weight:600; color:#6A7383; text-transform:uppercase; letter-spacing:0.5px; border-bottom:2px solid #D9D4C8; }
    .footer { font-size:12px; color:#6A7383; margin-top:32px; text-align:center; }
    .divider { border:none; border-top:1px solid #D9D4C8; margin:24px 0; }
  </style>
</head>
<body>
  <div class="container">
    <div class="card">
      ...
      <p>The following clients in <strong style="color:#0B1220">${opts.workspaceName}</strong> have not been contacted in <strong style="color:#0B1220">${opts.staleDays}+ days</strong>. Consider scheduling follow-ups to maintain engagement.</p>
      <table>
        <thead><tr>
      ...
      ${opts.staleClients.length > 25 ? `<p style="font-size:13px;color:#6A7383">Showing top 25 of ${opts.staleClients.length} stale clients.</p>` : ""}
      <hr class="divider" />
...
<head>
  ...
  <style>
    ...
    .logo { font-size:22px; font-weight:800; color:#0B1220; letter-spacing:-0.5px; margin-bottom:32px; }
    .logo span { color:#0B1220; }
    h1 { font-size:22px; font-weight:700; color:#0B1220; margin:0 0 12px; }
    p { font-size:15px; line-height:1.6; color:#4A5568; margin:0 0 16px; }
    .footer { font-size:12px; color:#6A7383; margin-top:32px; text-align:center; }
  </style>
</head>
<body>
  <div class="container">
    <div class="card">
      ...
      <p>Please find attached the latest performance report for <strong style="color:#0B1220">${opts.clientName}</strong>, prepared by <strong style="color:#0B1220">${opts.workspaceName}</strong>.</p>
      <p>This report includes a financial profile summary, projections, strategy history, and recent activity notes.</p>
      <p style="font-size:13px;color:#6A7383">This report was auto-generated by Russell Capital Systems™. If you have questions, please contact your advisor.</p>
    </div>
    <div class="footer">
      © ${new Date().getFullYear()} Russell Capital Systems™ · AI Wealth OS<br/>
      <a href="mailto:support@russellcapitalsystems.com" style="color:#6A7383">support@russellcapitalsystems.com</a>
...
export async function sendMeetingReminder(opts: MeetingReminderOptions): Promise<{ sent: boolean; reason?: string }> {
  ...
  const locationLine = opts.location ? `<p style="font-size:14px;color:#4A5568">📍 <strong style="color:#1C2230">${opts.location}</strong></p>` : "";
...
<head>
  ...
  <style>
    ...
    h1 { font-size:22px; font-weight:700; color:#0B1220; margin:0 0 12px; }
    p { font-size:15px; line-height:1.6; color:#4A5568; margin:0 0 16px; }
    .detail-box { background:#F7F4EE; border:1px solid #D9D4C8; border-radius:12px; padding:20px; margin:20px 0; }
    .detail-row { display:flex; justify-content:space-between; padding:6px 0; border-bottom:1px solid #D9D4C8; }
    .detail-row:last-child { border-bottom:none; }
    .detail-label { font-size:13px; color:#6A7383; font-weight:600; text-transform:uppercase; letter-spacing:0.5px; }
    .detail-value { font-size:14px; color:#0B1220; font-weight:500; }
    .reminder-badge { display:inline-block; background:#8A6A1F1A; border:1px solid #8A6A1F33; color:#8A6A1F; font-size:13px; font-weight:600; padding:4px 14px; border-radius:20px; margin-bottom:20px; }
    .footer { font-size:12px; color:#6A7383; margin-top:32px; text-align:center; }
  </style>
</head>
<body>
  <div class="container">
    <div class="card">
      ...
      <p>This is a reminder that you have a meeting scheduled with <strong style="color:#0B1220">${opts.clientName}</strong> tomorrow.</p>
      ...
      ${locationLine}
      <p style="font-size:13px;color:#6A7383">Log into Russell Capital Systems™ to review client details and prepare for your meeting.</p>
    </div>
    <div class="footer">
      © ${new Date().getFullYear()} Russell Capital Systems™ · AI Wealth OS<br/>
      <a href="mailto:support@russellcapitalsystems.com" style="color:#6A7383">support@russellcapitalsystems.com</a>
...
export async function sendLeaderboardDigest(opts: LeaderboardDigestOptions): Promise<{ sent: boolean; reason?: string }> {
  ...
  const tableRows = opts.entries.map((e, i) => {
    ...
    const bgColor = i % 2 === 0 ? "#F1EDE4" : "#FFFFFF";
    return `
      <tr style="background:${bgColor}">
        <td style="padding:10px 14px;text-align:center;font-size:16px">${medal}</td>
        <td style="padding:10px 14px;color:#0B1220;font-weight:600">${e.name}</td>
        <td style="padding:10px 14px;color:#1C2230;text-align:right">${e.score}</td>
        <td style="padding:10px 14px;color:#1C2230;text-align:right">${fmtMoney(e.aumManaged)}</td>
        <td style="padding:10px 14px;color:#1C2230;text-align:center">${e.dealsWon}</td>
        <td style="padding:10px 14px;color:#1C2230;text-align:right">${fmtMoney(e.closedValue)}</td>
        <td style="padding:10px 14px;color:#1C2230;text-align:center">${e.meetingsHeld}</td>
        <td style="padding:10px 14px;color:#1C2230;text-align:center">${e.clientCount}</td>
      </tr>`;
  }).join("");
...
<body style="margin:0;padding:0;background:#F7F4EE;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif">
  <table width="100%" cellpadding="0" cellspacing="0" style="max-width:700px;margin:0 auto;padding:24px">
    <tr><td>
      <div style="background:linear-gradient(135deg,#FFFFFF 0%,#D9D4C8 100%);border-radius:16px;border:1px solid #F1EDE4;overflow:hidden">
        <!-- Header -->
        <div style="padding:28px 32px;border-bottom:1px solid #F1EDE4;background:linear-gradient(90deg,#FFFFFF,#F1EDE4)">
          <div style="font-size:20px;font-weight:700;color:#0B1220;margin-bottom:4px">📊 Weekly Leaderboard Digest</div>
          <div style="font-size:13px;color:#4A5568">${opts.workspaceName} — ${dateStr}</div>
        </div>

        <!-- Summary stats -->
        <div style="padding:20px 32px;display:flex;gap:16px;border-bottom:1px solid #F1EDE4">
          <table width="100%" cellpadding="0" cellspacing="0"><tr>
            <td style="padding:12px;background:#F1EDE4;border-radius:8px;text-align:center;width:33%">
              <div style="font-size:11px;color:#4A5568;text-transform:uppercase;letter-spacing:1px;margin-bottom:4px">Total AUM</div>
              <div style="font-size:18px;font-weight:700;color:#0B1220">${fmtMoney(totalAum)}</div>
            </td>
            <td style="width:12px"></td>
            <td style="padding:12px;background:#F1EDE4;border-radius:8px;text-align:center;width:33%">
              <div style="font-size:11px;color:#4A5568;text-transform:uppercase;letter-spacing:1px;margin-bottom:4px">Deals Won</div>
              <div style="font-size:18px;font-weight:700;color:#3D6B4F">${totalDeals}</div>
            </td>
            <td style="width:12px"></td>
            <td style="padding:12px;background:#F1EDE4;border-radius:8px;text-align:center;width:33%">
              <div style="font-size:11px;color:#4A5568;text-transform:uppercase;letter-spacing:1px;margin-bottom:4px">Closed Value</div>
              <div style="font-size:18px;font-weight:700;color:#8A6A1F">${fmtMoney(totalClosed)}</div>
            </td>
          </tr></table>
        </div>
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;border-radius:8px;overflow:hidden">
            <thead>
              <tr style="background:#F1EDE4">
                <th style="padding:10px 14px;text-align:center;font-size:11px;color:#4A5568;text-transform:uppercase;letter-spacing:1px">#</th>
                <th style="padding:10px 14px;text-align:left;font-size:11px;color:#4A5568;text-transform:uppercase;letter-spacing:1px">Advisor</th>
                <th style="padding:10px 14px;text-align:right;font-size:11px;color:#4A5568;text-transform:uppercase;letter-spacing:1px">Score</th>
                <th style="padding:10px 14px;text-align:right;font-size:11px;color:#4A5568;text-transform:uppercase;letter-spacing:1px">AUM</th>
                <th style="padding:10px 14px;text-align:center;font-size:11px;color:#4A5568;text-transform:uppercase;letter-spacing:1px">Deals</th>
                <th style="padding:10px 14px;text-align:right;font-size:11px;color:#4A5568;text-transform:uppercase;letter-spacing:1px">Closed $</th>
                <th style="padding:10px 14px;text-align:center;font-size:11px;color:#4A5568;text-transform:uppercase;letter-spacing:1px">Meetings</th>
                <th style="padding:10px 14px;text-align:center;font-size:11px;color:#4A5568;text-transform:uppercase;letter-spacing:1px">Clients</th>
              </tr>
            </thead>
            <tbody>
              ${tableRows}
        ...
        <!-- Footer -->
        <div style="padding:20px 32px;border-top:1px solid #F1EDE4;text-align:center">
          <div style="font-size:12px;color:#4A5568">This is an automated weekly digest from ${opts.workspaceName}.</div>
          <div style="font-size:11px;color:#6A7383;margin-top:4px">Russell Capital Systems™</div>
        </div>
      </div>
    </td></tr>
  </table>
...
export async function sendStrategyNotification(opts: StrategyNotificationOptions): Promise<{ sent: boolean; reason?: string }> {
  ...
  const metricsRows = [
    opts.summary.iulNetCash != null ? { label: "IUL Net Cash Value", value: fmtDollar(opts.summary.iulNetCash), color: "#3D6B4F" } : null,
    opts.summary.propertyEquity != null ? { label: "Real Estate Equity", value: fmtDollar(opts.summary.propertyEquity), color: "#0B1220" } : null,
    opts.summary.rentalIncome != null ? { label: "Total Rental Income", value: fmtDollar(opts.summary.rentalIncome), color: "#8A6A1F" } : null,
    opts.summary.rothBalance != null ? { label: "Roth Balance", value: fmtDollar(opts.summary.rothBalance), color: "#8A6A1F" } : null,
    opts.summary.netWorth != null ? { label: "Estimated Net Worth", value: fmtDollar(opts.summary.netWorth), color: "#0B1220" } : null,
  ].filter(Boolean) as { label: string; value: string; color: string }[];

  const metricsHtml = metricsRows.map(r =>
    `<div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid #D9D4C8">
      <span style="font-size:14px;color:#4A5568">${r.label}</span>
...
<head>
  ...
  <style>
    ...
    h1 { font-size:22px; font-weight:700; color:#0B1220; margin:0 0 12px; }
    p { font-size:15px; line-height:1.6; color:#4A5568; margin:0 0 16px; }
    .strategy-badge { display:inline-block; background:#3D6B4F1A; border:1px solid #3D6B4F33; color:#3D6B4F; font-size:13px; font-weight:600; padding:5px 16px; border-radius:20px; margin-bottom:20px; }
    .carrier-badge { display:inline-block; background:#0B12201A; border:1px solid #0B122033; color:#0B1220; font-size:12px; font-weight:600; padding:3px 12px; border-radius:16px; margin-left:8px; }
    .metrics-box { background:#F7F4EE; border:1px solid #D9D4C8; border-radius:12px; padding:16px 20px; margin:20px 0; }
    .cta { display:block; background:#3D6B4F; color:#0B1220 !important; text-decoration:none; font-weight:700; font-size:15px; text-align:center; padding:14px 28px; border-radius:10px; margin:28px 0; }
    .cta:hover { background:#3D6B4F; }
    .notes-box { background:#8A6A1F0D; border:1px solid #8A6A1F33; border-radius:10px; padding:14px 18px; margin:16px 0; }
    .footer { font-size:12px; color:#6A7383; margin-top:32px; text-align:center; }
    .divider { border:none; border-top:1px solid #D9D4C8; margin:24px 0; }
  </style>
</head>
<body>
  <div class="container">
    <div class="card">
      ...
      <p>Hello${opts.toName ? ` ${opts.toName}` : ""},</p>
      <p>Your advisor <strong style="color:#0B1220">${opts.advisorName}</strong> has prepared a new financial strategy analysis for you. Here's a summary of the projected outcomes:</p>
      <div class="metrics-box">
        ${metricsHtml}
      </div>
      ${opts.notes ? `<div class="notes-box"><div style="font-size:12px;color:#8A6A1F;font-weight:600;margin-bottom:6px">Advisor Notes</div><p style="font-size:14px;color:#1C2230;margin:0">${opts.notes}</p></div>` : ""}
      <a href="${opts.portalUrl}" class="cta">View Full Analysis →</a>
      <p style="font-size:13px">Or copy this link into your browser:</p>
      <div style="background:#F7F4EE;border:1px solid #D9D4C8;border-radius:8px;padding:12px 16px;font-size:12px;color:#4A5568;word-break:break-all;margin-bottom:24px">${opts.portalUrl}</div>
      <hr class="divider" />
...
export async function sendProjectionFollowUp(opts: FollowUpEmailOptions): Promise<{ sent: boolean; reason?: string }> {
  ...
  const bodyText = isReminder
    ? `Just a quick reminder — your personalized financial strategy analysis from <strong style="color:#0B1220">${opts.advisorName}</strong> is still available for review. Don't miss the opportunity to explore your projected outcomes.`
    : `Your advisor <strong style="color:#0B1220">${opts.advisorName}</strong> recently shared a personalized financial strategy analysis with you. Take a moment to review your projected outcomes and see how this strategy could work for you.`;
...
<head>
  ...
  <style>
    ...
    .cta { display:block; background:#3D6B4F; color:#0B1220 !important; text-decoration:none; font-weight:700; font-size:15px; text-align:center; padding:14px 28px; border-radius:10px; margin:28px 0; }
    .cta:hover { background:#3D6B4F; }
    .book-cta { display:block; background:transparent; border:1px solid #0B1220; color:#0B1220 !important; text-decoration:none; font-weight:700; font-size:14px; text-align:center; padding:12px 24px; border-radius:10px; margin:12px 0; }
    .footer { font-size:12px; color:#6A7383; margin-top:32px; text-align:center; }
    .divider { border:none; border-top:1px solid #D9D4C8; margin:24px 0; }
  </style>
</head>
<body>
  <div class="container">
    <div class="card">
      ...
      <p>Hello${opts.toName ? ` ${opts.toName}` : ""},</p>
      <p>${bodyText}</p>
      ${opts.projectionSummary ? `<div style="background:#F7F4EE;border:1px solid #D9D4C8;border-radius:12px;padding:16px 20px;margin:20px 0;font-size:14px;color:#1C2230">${opts.projectionSummary}</div>` : ""}
      <a href="${opts.shareUrl}" class="cta">View Your Strategy Analysis →</a>
      <hr class="divider" />
      <p style="font-size:14px;color:#0B1220;font-weight:600">Ready to discuss your options?</p>
      ...
      <hr class="divider" />
      <p style="font-size:12px;color:#6A7383">This link will expire in 30 days from the original share date. If you have any questions, please reach out to your advisor directly.</p>
    </div>
    <div class="footer">
      © ${new Date().getFullYear()} Russell Capital Systems™ · AI Wealth OS<br/>
      <a href="mailto:support@russellcapitalsystems.com" style="color:#6A7383">support@russellcapitalsystems.com</a>
...
<head>
  ...
  <style>
    ...
    h1 { font-size:22px; font-weight:700; color:#0B1220; margin:0 0 12px; }
    p { font-size:15px; line-height:1.6; color:#4A5568; margin:0 0 16px; }
    .detail-box { background:#F7F4EE; border:1px solid #D9D4C8; border-radius:12px; padding:16px 20px; margin:20px 0; }
    .detail-row { display:flex; justify-content:space-between; padding:8px 0; border-bottom:1px solid #D9D4C8; }
    .detail-label { font-size:13px; color:#4A5568; }
    .detail-value { font-size:13px; font-weight:600; color:#0B1220; }
    .footer { font-size:12px; color:#6A7383; margin-top:32px; text-align:center; }
  </style>
</head>
<body>
  <div class="container">
    <div class="card">
      ...
      <p>A formal quote request has been submitted for <strong style="color:#0B1220">${opts.clientName}</strong>.</p>
      <div class="detail-box">
      ...
      <p style="font-size:13px;color:#6A7383">This is an automated notification. The quote request is now pending review.</p>
    </div>
    <div class="footer">
      © ${new Date().getFullYear()} Russell Capital Systems™ · AI Wealth OS<br/>
      <a href="mailto:support@russellcapitalsystems.com" style="color:#6A7383">support@russellcapitalsystems.com</a>
...
<head>
  ...
  <style>
    ...
    .container { max-width: 560px; margin: 40px auto; padding: 0 20px; }
    .card { background: #FFFFFF; border: 1px solid #D9D4C8; border-radius: 16px; padding: 40px; }
    .logo { font-size: 22px; font-weight: 800; color: #0B1220; letter-spacing: -0.5px; margin-bottom: 8px; }
    .badge { font-size: 48px; text-align: center; margin: 20px 0; }
    .score-bar { background: #F1EDE4; border-radius: 12px; height: 24px; overflow: hidden; margin: 16px 0; }
    .score-fill { background: linear-gradient(90deg, #3D6B4F, #3D6B4F); height: 100%; border-radius: 12px; transition: width 0.5s; }
    .tip-box { background: #F1EDE4; border: 1px solid #F1EDE4; border-radius: 12px; padding: 20px; margin: 20px 0; }
    .cta { display: inline-block; background: linear-gradient(135deg, #3D6B4F, #3D6B4F); color: #0B1220; padding: 14px 32px; border-radius: 10px; text-decoration: none; font-weight: 700; font-size: 15px; margin-top: 24px; }
    .footer { text-align: center; color: #6A7383; font-size: 12px; margin-top: 32px; }
    h2 { color: #0B1220; margin: 0 0 8px; }
    p { line-height: 1.6; margin: 8px 0; }
    .highlight { color: #3D6B4F; font-weight: 700; }
    .level-up { background: linear-gradient(135deg, #8A6A1F, #8A6A1F); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-weight: 800; }
  </style>
</head>
<body>
  <div class="container">
    <div class="card">
      ...
      <p style="color: #6A7383; font-size: 13px; margin-bottom: 24px;">Your Weekly Financial Journey Update</p>
      ...
      <div style="text-align: center;">
        <p style="font-size: 14px; color: #6A7383;">Current Level</p>
        <p style="font-size: 20px; font-weight: 800; color: #0B1220;">${opts.levelName}</p>
        <p style="font-size: 36px; font-weight: 800; color: #3D6B4F;">${opts.currentScore}<span style="font-size: 16px; color: #6A7383;">/100</span></p>
      </div>

      <div class="score-bar">
        <div class="score-fill" style="width: ${progressPct}%;"></div>
      </div>
      <p style="text-align: center; font-size: 13px; color: #6A7383;">
      ...
      ${opts.topGoal ? `
      <div class="tip-box">
        <p style="font-size: 13px; color: #8A6A1F; font-weight: 700; margin-bottom: 8px;">🎯 YOUR TOP GOAL</p>
        <p style="color: #0B1220; font-weight: 600;">${opts.topGoal}</p>
        <p style="font-size: 13px;">Stay focused — every smart decision brings you closer!</p>
      </div>` : ""}

      ${opts.topRecommendation ? `
      <div class="tip-box">
        <p style="font-size: 13px; color: #3D6B4F; font-weight: 700; margin-bottom: 8px;">💡 RECOMMENDED ACTION</p>
        <p style="color: #0B1220; font-weight: 600;">${opts.topRecommendation}</p>
        <p style="font-size: 13px;">This could boost your score by <span class="highlight">+5 points</span> or more!</p>
      </div>` : ""}

      <div class="tip-box">
        <p style="font-size: 13px; color: #8A6A1F; font-weight: 700; margin-bottom: 8px;">📚 WEEKLY TIP</p>
        <p>${opts.weeklyTip}</p>
      </div>
      ...
      <p style="text-align: center; font-size: 13px; color: #6A7383; margin-top: 24px;">
...
export async function sendSessionRatingEmail(opts: SessionRatingEmailOptions): Promise<{ sent: boolean; reason?: string }> {
  ...
  const ratingColor = opts.rating >= 8 ? "#3D6B4F" : opts.rating >= 5 ? "#8A6A1F" : "#A3322D";
...
<head>
  ...
  <style>
    ...
    .card { background: #FFFFFF; border: 1px solid #D9D4C8; border-radius: 16px; padding: 40px; }
    .logo { font-size: 22px; font-weight: 800; color: #0B1220; letter-spacing: -0.5px; margin-bottom: 8px; }
    .rating-circle { width: 80px; height: 80px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 20px auto; font-size: 32px; font-weight: 800; color: #0B1220; }
    .section { background: #F1EDE4; border: 1px solid #F1EDE4; border-radius: 12px; padding: 20px; margin: 16px 0; }
    .cta { display: inline-block; background: linear-gradient(135deg, #3D6B4F, #3D6B4F); color: #0B1220; padding: 14px 32px; border-radius: 10px; text-decoration: none; font-weight: 700; font-size: 15px; margin-top: 24px; }
    .footer { text-align: center; color: #6A7383; font-size: 12px; margin-top: 32px; }
    h2 { color: #0B1220; margin: 0 0 8px; }
    h3 { color: #0B1220; margin: 0 0 12px; font-size: 14px; }
    p { line-height: 1.6; margin: 8px 0; }
...
<body>
  <div class="container">
    <div class="card">
      <div class="logo">Russell Capital Systems™</div>
      <p style="color: #6A7383; font-size: 13px; margin-bottom: 24px;">Session Summary — ${opts.sessionDate}</p>
      ...
      <div class="section">
        <h3>📈 Score Impact</h3>
        <p>This session ${opts.scoreImpact >= 0 ? "boosted" : "impacted"} your financial score by <strong style="color: ${opts.scoreImpact >= 0 ? "#3D6B4F" : "#A3322D"};">${opts.scoreImpact >= 0 ? "+" : ""}${opts.scoreImpact.toFixed(1)}</strong> points.</p>
      </div>
...
<html>
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>
<body style="margin:0;padding:0;background:#F7F4EE;font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;">
<table width="100%" cellpadding="0" cellspacing="0" style="background:#F7F4EE;padding:32px 16px;">
<tr><td align="center">
<table width="600" cellpadding="0" cellspacing="0" style="background:#F1EDE4;border-radius:12px;overflow:hidden;border:1px solid #D9D4C8;">
  <!-- Header -->
  <tr><td style="background:linear-gradient(135deg,#EEF1F5,#EEF1F5);padding:24px 32px;">
    <h1 style="margin:0;color:#fff;font-size:20px;font-weight:700;">Russell Capital Systems™</h1>
    <p style="margin:4px 0 0;color:#3D6B4F;font-size:12px;text-transform:uppercase;letter-spacing:1px;">${opts.campaignName}</p>
  </td></tr>
  <!-- Body -->
  <tr><td style="padding:32px;color:#D9D4C8;font-size:15px;line-height:1.7;">
    ${opts.toName ? `<p style="margin:0 0 16px;color:#6A7383;">Dear ${opts.toName},</p>` : ""}
    <div style="color:#D9D4C8;">${opts.body}</div>
  </td></tr>
  <!-- Footer -->
  <tr><td style="padding:16px 32px 24px;border-top:1px solid #D9D4C8;">
    <p style="margin:0;color:#6A7383;font-size:11px;text-align:center;">
...
<html>
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head>
<body style="margin:0;padding:0;background-color:#F7F4EE;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#F7F4EE;padding:40px 20px;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background-color:#F1EDE4;border-radius:12px;overflow:hidden;">
        <tr><td style="background:linear-gradient(135deg,#3D6B4F,#3D6B4F);padding:32px;text-align:center;">
          <h1 style="color:#0B1220;margin:0;font-size:24px;font-weight:700;">Russell Capital Systems™</h1>
          <p style="color:#EEF1F5;margin:8px 0 0;font-size:14px;">Password Reset Request</p>
        </td></tr>
        <tr><td style="padding:32px;">
          <p style="color:#D9D4C8;font-size:16px;line-height:1.6;margin:0 0 16px;">Hi ${opts.userName},</p>
          <p style="color:#6A7383;font-size:15px;line-height:1.6;margin:0 0 24px;">We received a request to reset your password. Click the button below to create a new password. This link will expire in 1 hour.</p>
          <table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:8px 0 24px;">
            <a href="${resetUrl}" style="display:inline-block;background:linear-gradient(135deg,#3D6B4F,#3D6B4F);color:#0B1220;text-decoration:none;padding:14px 32px;border-radius:8px;font-size:16px;font-weight:600;">Reset Password</a>
          </td></tr></table>
          <p style="color:#6A7383;font-size:13px;line-height:1.6;margin:0 0 8px;">If the button doesn't work, copy and paste this link into your browser:</p>
          <p style="color:#3E5F8A;font-size:13px;word-break:break-all;margin:0 0 24px;">${resetUrl}</p>
          <p style="color:#6A7383;font-size:13px;line-height:1.6;margin:0;">If you didn't request this reset, you can safely ignore this email. Your password will remain unchanged.</p>
        </td></tr>
        <tr><td style="padding:24px 32px;border-top:1px solid #D9D4C8;text-align:center;">
          <p style="color:#6A7383;font-size:12px;margin:0;">Russell Capital Systems™ &bull; www.RussellCapitalSystems.com</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
export async function sendDriftAlertEmail(opts: DriftAlertEmailOptions): Promise<{ sent: boolean; reason?: string }> {
  ...
  const urgencyColor = opts.driftScore >= 20 ? "#A3322D" : opts.driftScore >= 15 ? "#8A6A1F" : "#0B1220";
...
<div style="max-width:600px;margin:0 auto;padding:32px 24px">
  <div style="text-align:center;margin-bottom:24px">
    <div style="display:inline-block;background:#3D6B4F;color:#fff;font-weight:800;font-size:14px;padding:8px 16px;border-radius:8px">RCS</div>
  ...
  <div style="background:#D9D4C8;border-radius:12px;padding:24px;margin-bottom:16px">
    <p style="color:#4A5568;margin:0 0 8px;font-size:14px">Client</p>
    <p style="color:#fff;margin:0 0 16px;font-size:18px;font-weight:700">${opts.clientName}</p>
    <div style="display:flex;justify-content:space-between;padding:12px 0;border-bottom:1px solid #F1EDE4">
      <span style="color:#4A5568;font-size:14px">Previous Score</span>
      <span style="color:#fff;font-weight:700;font-size:14px">${opts.previousScore}</span>
    </div>
    <div style="display:flex;justify-content:space-between;padding:12px 0;border-bottom:1px solid #F1EDE4">
      <span style="color:#4A5568;font-size:14px">Current Score</span>
      <span style="color:#fff;font-weight:700;font-size:14px">${opts.currentScore}</span>
    </div>
    <div style="display:flex;justify-content:space-between;padding:12px 0;border-bottom:1px solid #F1EDE4">
      <span style="color:#4A5568;font-size:14px">Drift Magnitude</span>
      <span style="color:${urgencyColor};font-weight:700;font-size:14px">${opts.driftScore} points</span>
    </div>
    <div style="display:flex;justify-content:space-between;padding:12px 0">
      <span style="color:#4A5568;font-size:14px">Direction</span>
      <span style="color:#fff;font-weight:700;font-size:14px">${directionLabel}</span>
    </div>
  </div>
  <div style="background:#F1EDE4;border-left:4px solid ${urgencyColor};border-radius:8px;padding:16px;margin-bottom:16px">
    <p style="color:#fff;margin:0;font-size:14px;font-weight:600">Recommended Action</p>
    <p style="color:#4A5568;margin:8px 0 0;font-size:13px">${opts.driftScore >= 15 ? "Schedule a reassessment meeting with this client. Their risk tolerance has shifted significantly and current allocations may no longer be appropriate." : "Monitor this client closely. Consider discussing any life changes that may have influenced their risk perspective."}</p>
  </div>
  ${opts.dashboardUrl ? `<div style="text-align:center;margin-top:24px"><a href="${opts.dashboardUrl}" style="display:inline-block;background:#3D6B4F;color:#fff;text-decoration:none;padding:12px 32px;border-radius:8px;font-weight:700;font-size:14px">View Client Profile</a></div>` : ""}
  <p style="color:#6A7383;font-size:11px;text-align:center;margin-top:32px">Russell Capital Systems™ — Turn Capital Into Income&trade;</p>
</div>
</body></html>`;
...
export async function sendWithdrawalEmail(opts: WithdrawalEmailOptions): Promise<{ sent: boolean; reason?: string }> {
  ...
  if (escalationLevel === "gentle") {
    contentBlocks = `<div style="background:#D9D4C8;border-radius:12px;padding:24px;margin-bottom:16px">
      <h3 style="color:#fff;margin:0 0 12px;font-size:16px">It's been a while, ${firstName}</h3>
      <p style="color:#4A5568;margin:0;font-size:14px;line-height:1.6">You haven't logged in for ${Math.round(hoursInactive)} hours. Your tools are ready and waiting &mdash; new market data has been processed and your calculators are primed with the latest rates.</p>
    </div>`;
  } else if (escalationLevel === "urgent") {
    contentBlocks = `<div style="background:#D9D4C8;border-radius:12px;padding:24px;margin-bottom:16px">
      <h3 style="color:#8A6A1F;margin:0 0 12px;font-size:16px">Your Progress Is At Risk</h3>
      <p style="color:#4A5568;margin:0 0 16px;font-size:14px;line-height:1.6">You've been away for ${Math.round(hoursInactive)} hours. Here's what needs your attention:</p>
      <div style="border-left:3px solid #8A6A1F;padding-left:12px;margin-bottom:12px">
    ...
    contentBlocks = `<div style="background:#D9D4C8;border-radius:12px;padding:24px;margin-bottom:16px">
      <h3 style="color:#A3322D;margin:0 0 12px;font-size:16px">You're Falling Behind</h3>
      <p style="color:#4A5568;margin:0 0 16px;font-size:14px;line-height:1.6">It's been ${Math.round(hoursInactive)} hours since your last session. While you've been away:</p>
      <div style="background:#F1EDE4;border-radius:8px;padding:16px;margin-bottom:12px">
        ...
        ${currentStreak ? `<p style="color:#A3322D;margin:0;font-size:13px;font-weight:700">Your ${currentStreak}-day streak is GONE if you don't act now</p>` : ""}
      </div>
    </div>`;
  }
...
<div style="max-width:600px;margin:0 auto;padding:32px 24px">
  ...
  <div style="text-align:center;margin-top:24px">
    <a href="${dashboardUrl}" style="display:inline-block;background:#3D6B4F;color:#fff;text-decoration:none;padding:14px 36px;border-radius:8px;font-weight:700;font-size:15px">Return to Dashboard</a>
  </div>
  <p style="color:#6A7383;font-size:11px;text-align:center;margin-top:32px">Russell Capital Systems&trade;</p>
</div></body></html>`;
