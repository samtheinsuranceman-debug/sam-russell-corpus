...
const STRATEGY_NOTES: StrategyNote[] = [
  { match: /mortgage killer|recycl|cycle/i, mechanism: "Surplus cash after living costs is directed at the mortgage principal in cycles of roughly six to seven years. Each cycle ends with a paid-off property; the freed payment becomes the next cycle's surplus, so the machine accelerates rather than repeats.", answers: "It turns the objection into arithmetic: each dollar of principal paid avoids future interest at the mortgage rate, and in the model every cycle adds an asset that pays rent.", risk: "The cycle slows when surplus cash falls (job change, expense shock) and pauses when property values fall; it does not reverse, because no cycle relies on selling.", source: "Freddie Mac PMMS mortgage rate history; FHFA House Price Index; the firm's Mortgage Killer method (proprietary method)." },
  ...
  { match: /income floor|annuit|guaranteed income|income for life|1035/i, mechanism: "A portion of retirement assets is exchanged for a contractual income for life, sized to cover fixed expenses. With the floor in place the rest of the portfolio can stay invested through bad years without forced selling.", answers: "It answers 'the market will beat this' and 'what if it crashes': the contractual income is set by the contract, not the market, subject to the insurer's claims-paying ability.", risk: "Guarantees depend on the carrier's claims-paying ability; surrender periods limit access; inflation erodes a fixed payment unless the rider indexes it. A 1035 exchange must be like-kind to stay untaxed.", source: "IRC §1035; NAIC annuity suitability model regulation; carrier ratings (A.M. Best, S&P, Moody's) on file." },
...
function sayThis(strategy: string, first: string): string {
  if (/mortgage/i.test(strategy)) return `${first}, the mortgage is not a bill, it is the first machine. Every dollar we put on it avoids interest at the mortgage rate, and in the model, the day it is gone the payment funds the next property.`;
  ...
  if (/IUL|policy|trust/i.test(strategy)) return `This is life insurance, not an investment. Its loans can be income-tax-free if the policy is not a MEC and stays in force, and your state may protect some of its cash value from creditors, paid for by money the cycle freed.`;
