...
describe("managed primary-port safeguards", () => {
  it("preserves all 222 primary paths after additive unification", () => {
    ...
    expect(currentRoutes.size).toBe(321); // 232 + the original platform routes + /for and /for/:slug + /portal/interior // master: +1 /portal/voice (Voice Studio), +1 /portal/whisperer, +3 role dashboards, +1 /portal/chain // branch: +39 calculators and tools restored in the catalogue pass (page files that existed on disk but were never routed), +1 /portal/qbi-optimizer, +2 /portal/mortgage-ledger and /portal/liquidity-routes // +1: /portal/how-a-figure-is-made, the provenance page // +2: /portal/alt-credit and /portal/alt-credit/:slug, the alternative-lines-of-credit tab // +1: /portal/genome-strategies, the genome strategy-fit formulary // +1: /portal/household-genome, the two-spouse pairing protocol // +1: /portal/infinite-banking, the cycle engine // +4: /portal/mechanisms plus the three per-mechanism views (:slug, :slug/providers, :slug/sequences) // +2: /portal/sequence-planner and /portal/thresholds // +1: /portal/recin, the real-estate intelligence workspace (base consolidation harvest)
  });
