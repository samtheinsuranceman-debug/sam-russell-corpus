# DRAFT PROVISIONAL PATENT APPLICATION — AQAL-C-12

**Title of Invention:** Voice-Driven Staged Weakness Coaching
...
**Combinator.** The combination executes as the `voiceStagedWeaknessCoaching` combinator (server/patents/combinations/index.ts), registered in the sister-combination registry (server/patents/combinations/registry.ts) and invoked in the production scoring pass over the real outputs of each scoring run.
