# DRAFT PROVISIONAL PATENT APPLICATION — JQ-L1-07

**Title of Invention:** Controlling-Weakness Argmin Diagnostic
...
The invention identifies the single line that most constrains a profile by a deterministic argmin over the 32-dimension vector and quantifies its constraint impact as the weakest line's distance below the profile mean, operationalizing limiting-factor theory (Liebig's minimum; the O-ring result) as a constant-time computation.

## Abstract

Controlling-Weakness Argmin Diagnostic: a computer-implemented system that identifies the single line that most constrains a profile by a deterministic argmin over the 32-dimension vector and quantifies its constraint impact as the weakest line's distance below the profile mean, operationalizing limiting-factor theory (Liebig's minimum; the O-ring resul…

## Technical Improvement (§101 Positioning)

The claims are directed not to an abstract idea but to a specific technical improvement in the functioning of the computing system: elevates the minimum rather than the mean — the diagnostic that average-based scoring structurally cannot produce — at negligible computational cost. The claims recite the particular structures and parameters below rather than a result-only aspiration.

## Detailed Description

The engine identifies the single line that most constrains a profile by a deterministic argmin over the 32-dimension vector and quantifies its constraint impact as the weakest line's distance below the profile mean, operationalizing limiting-factor theory (Liebig's minimum; the O-ring result) as a constant-time computation.

*Implementation:* `server/scoring/controllingWeakness.ts`.

## Operating Parameters (Enablement Detail)

argmin over fixed 32-dim vector; constraintImpact = clamp(mean - min, 0..1); result committed to the ledger; measured 0.11 microseconds per operation.

## Measured Performance

- 8,745,295 argmin operations/sec on the 32-line vector.
...
The following contrast reflects the applicant's engineering understanding, not an assertion that no prior art exists: Composite/average instruments: report the mean, hide the binding constraint.
...
1. A computer-implemented method of improving the functioning of a cognitive-assessment computing system, the method comprising: (a) identifies the single line that most constrains a profile by a deterministic argmin over the 32-dimension vector and quantifies its constraint impact as the weakest line's distance below the profile mean, operationalizing limiting-factor theory (Liebig's minimum; the O-ring result) as a constant-time computation; and (b) presenting the result with a disclosure of the assumptions and inputs that produced it.
