# DRAFT PROVISIONAL PATENT APPLICATION — JQ-L1-09

**Title of Invention:** Versioned Population Norming with Public Changelog
...
The invention freezes each population-norming curve as an immutable versioned snapshot; every stored score is stamped with its norming version so recomputation under that version reproduces the identical rarity at any later time; a public changelog enumerates every version and an as-of-version endpoint recomputes rarities under any frozen snapshot without mutating stored data.

## Abstract

Versioned Population Norming with Public Changelog: a computer-implemented system that freezes each population-norming curve as an immutable versioned snapshot; every stored score is stamped with its norming version so recomputation under that version reproduces the identical rarity at any later time; a public changelog enumerates every version and an as-of-version…

## Technical Improvement (§101 Positioning)

The claims are directed not to an abstract idea but to a specific technical improvement in the functioning of the computing system: reproducibility as an architectural guarantee: any historical score can be traced and re-derived under the exact population model that produced it — regulatory-grade transparency. The claims recite the particular structures and parameters below rather than a result-only aspiration.

## Detailed Description

The engine freezes each population-norming curve as an immutable versioned snapshot; every stored score is stamped with its norming version so recomputation under that version reproduces the identical rarity at any later time; a public changelog enumerates every version and an as-of-version endpoint recomputes rarities under any frozen snapshot without mutating stored data.

*Implementation:* `server/scoring/norming.ts; GET /api/norms/changelog; GET /api/norms/recompute; norm_version ledger entries`.

## Operating Parameters (Enablement Detail)

Snapshot = {version, method, description, frozen score-to-rarity curve}; active-version boot entry appended to the ledger only on change; historical scores never re-normed in place.

## Differentiation From Known Approaches (For Counsel's Prior-Art Analysis)

The following contrast reflects the applicant's engineering understanding, not an assertion that no prior art exists: Instruments that silently re-norm: historical scores become irreproducible; no public version record.
...
1. A computer-implemented method of improving the functioning of a cognitive-assessment computing system, the method comprising: (a) freezes each population-norming curve as an immutable versioned snapshot; every stored score is stamped with its norming version so recomputation under that version reproduces the identical rarity at any later time; a public changelog enumerates every version and an as-of-version endpoint recomputes rarities under any frozen snapshot without mutating stored data; and (b) presenting the result with a disclosure of the assumptions and inputs that produced it.
