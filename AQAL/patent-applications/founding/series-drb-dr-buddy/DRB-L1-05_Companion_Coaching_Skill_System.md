# DRAFT PROVISIONAL PATENT APPLICATION — DRB-L1-05

**Title of Invention:** Companion Coaching Skill System
...
The invention packages the companion's coaching competence as a versioned skill: a skill definition document plus reference frameworks the agent loads to deliver structured success-coaching, separating coaching methodology (versioned documents) from the agent runtime.

## Abstract

Companion Coaching Skill System: a computer-implemented system that packages the companion's coaching competence as a versioned skill: a skill definition document plus reference frameworks the agent loads to deliver structured success-coaching, separating coaching methodology (versioned documents) from the agent runtime.

## Technical Improvement (§101 Positioning)

The claims are directed not to an abstract idea but to a specific technical improvement in the functioning of the computing system: coaching methodology as versioned, auditable content decoupled from the model — the methodology improves without retraining. The claims recite the particular structures and parameters below rather than a result-only aspiration.

## Detailed Description

The engine packages the companion's coaching competence as a versioned skill: a skill definition document plus reference frameworks the agent loads to deliver structured success-coaching, separating coaching methodology (versioned documents) from the agent runtime.

*Implementation:* `coaching_system/SKILL.md + references/ frameworks`.

## Operating Parameters (Enablement Detail)

SKILL.md contract + reference-framework corpus; methodology updates ship as document versions, not code deployments.

## Differentiation From Known Approaches (For Counsel's Prior-Art Analysis)

The following contrast reflects the applicant's engineering understanding, not an assertion that no prior art exists: Prompt-embedded coaching heuristics: unversioned, unauditable.
...
1. A computer-implemented method of improving the functioning of a persistent multi-session artificial-intelligence agent system, the method comprising: (a) packages the companion's coaching competence as a versioned skill: a skill definition document plus reference frameworks the agent loads to deliver structured success-coaching, separating coaching methodology (versioned documents) from the agent runtime; and (b) presenting the result with a disclosure of the assumptions and inputs that produced it.
