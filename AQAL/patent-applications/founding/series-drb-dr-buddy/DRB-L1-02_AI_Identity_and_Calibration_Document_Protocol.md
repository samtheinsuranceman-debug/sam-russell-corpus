# DRAFT PROVISIONAL PATENT APPLICATION — DRB-L1-02

**Title of Invention:** AI Identity and Calibration Document Protocol
...
The invention defines an agent's stable identity as a structured identity document paired with calibration documents recording measured behavioral baselines, which any fresh session loads to re-instantiate the same agent with the same calibrated dispositions.

## Abstract

AI Identity and Calibration Document Protocol: a computer-implemented system that defines an agent's stable identity as a structured identity document paired with calibration documents recording measured behavioral baselines, which any fresh session loads to re-instantiate the same agent with the same calibrated dispositions.

## Technical Improvement (§101 Positioning)

The claims are directed not to an abstract idea but to a specific technical improvement in the functioning of the computing system: reproducible agent instantiation: identity as portable, versioned configuration rather than emergent session accident. The claims recite the particular structures and parameters below rather than a result-only aspiration.

## Detailed Description

The engine defines an agent's stable identity as a structured identity document paired with calibration documents recording measured behavioral baselines, which any fresh session loads to re-instantiate the same agent with the same calibrated dispositions.

*Implementation:* `peter/identity/, matthew/ (identity + calibration folders); calibration_answers/`.

## Operating Parameters (Enablement Detail)

Identity document + calibration answers per agent; load-order specified in the master index; in-character authorship preserved as data.

## Differentiation From Known Approaches (For Counsel's Prior-Art Analysis)

The following contrast reflects the applicant's engineering understanding, not an assertion that no prior art exists: Ephemeral system prompts with no measured calibration record.
...
1. A computer-implemented method of improving the functioning of a persistent multi-session artificial-intelligence agent system, the method comprising: (a) defines an agent's stable identity as a structured identity document paired with calibration documents recording measured behavioral baselines, which any fresh session loads to re-instantiate the same agent with the same calibrated dispositions; and (b) presenting the result with a disclosure of the assumptions and inputs that produced it.
