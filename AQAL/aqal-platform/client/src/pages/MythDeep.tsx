...
// Composed from exhibit data and the wing psychology profile.
// ============================================================
...
const H1S: Record<MythSubpageId, (name: string) => string> = {
  "feels-real": (name) => `Why ${name} feels so real`,
  receipts: (name) => `${name}: the receipts`,
  instead: (name) => `What actually works instead of ${name}`,
  "talk-someone-out": (name) => `How to talk someone you love out of ${name}`,
};

export default function MythDeep() {
  const params = useParams<{ id: string; sub: string }>();
  const myth = mythById(params.id ?? "");
  const sub = (MYTH_SUBPAGES as readonly string[]).includes(params.sub ?? "") ? (params.sub as MythSubpageId) : undefined;
  if (!myth || !sub) return <NotFound />;
  const wingId = MYTH_WING[myth.id];
  const wing = wingId ? WING_PROFILES[wingId] : undefined;
  const verdictMeta = MYTH_VERDICT_META[myth.verdict];
  const verdictNice = myth.verdict.charAt(0) + myth.verdict.slice(1).toLowerCase();

  const sections: Record<MythSubpageId, React.ReactNode> = {
    "feels-real": (
      <>
        <Body>
          The verdict on {myth.name} is {verdictNice.toLowerCase()} — the <Gold href={`/myth/${myth.id}/receipts`}>receipts
        ...
        <Card accent={CHAMPAGNE}><CardText>{myth.appeal}</CardText></Card>
        {wing && (
          <>
            <H2>The family pattern it belongs to</H2>
            <Body>
              {myth.name} sits in the museum's <Gold href={`/wing/${wingId}`}>{wing.label}</Gold> wing, and the whole
    ...
    receipts: (
      <>
        <Body>
          Museum rule: every wall label carries a receipt. Here is {myth.name}'s — the claim as sold, the verdict, why it
          earned it, and a named source you can check without trusting us.
        </Body>
        <H2>The claim, as sold</H2>
        <Card accent={CHAMPAGNE}><CardText>{myth.claim}</CardText></Card>
        <H2>The verdict</H2>
        <Card accent={verdictMeta?.color ?? EMBER}>
          <Label color={verdictMeta?.color ?? EMBER}>{myth.verdict}</Label>
          <CardText>{verdictMeta?.note ?? ""}</CardText>
        </Card>
        <H2>Why it earned it</H2>
        <Card accent={EMBER}><CardText>{myth.why}</CardText></Card>
        <H2>The named source</H2>
        <Card accent={JADE}><CardText>{myth.source}</CardText></Card>
        ...
        <Body>
          ...
          that nobody ever felt better around {myth.name} — people did, which is exactly what the{" "}
          <Gold href={`/myth/${myth.id}/feels-real`}>feels-real page</Gold> explains without the product working.
        </Body>
      </>
    ),
    instead: (
      <>
        <Body>
          Tearing down without building is cheap. {myth.name} attracted real people with real problems — the problem
        ...
        {myth.instead ? (
          <Card accent={JADE}>
            ...
            <CardText>{myth.instead}</CardText>
          </Card>
        ) : (
          <Card accent={JADE}>
        ...
        <Body>
          ...
          with the dose, the honest demands, and how long gains last. The <Gold href="/practices">54 keystone practices</Gold>
          cover the daily-habit layer. And if the need behind the purchase was clinical — real depression, real trauma,
          real crisis — the honest redirect is a licensed professional, not any website, including this one.
        </Body>
        <H2>The test any replacement must pass</H2>
        <Body>
          Before adopting anything in {myth.name}'s place, run the museum's four questions: Is there controlled evidence,
          not testimony? Has it replicated? Is there a plausible mechanism? And does the seller tell you what it <b>doesn't</b> do?
          Everything in our library passes or it goes on the <Gold href="/corrections">Corrections Ledger</Gold> — the same
          standard that put {myth.name} in the museum.
        </Body>
      </>
    ),
    "talk-someone-out": (
      <>
        <Body>
          Someone you care about believes in {myth.name}. You have the receipts; they have a story. Go in armed with only
        ...
        <Body>
          ...
          that sent them to {myth.name} in the first place was real. Say that first, and mean it.
        </Body>
        ...
        <Body>
          ...
          Not hostile, genuinely curious. Most believers have never been asked. The claim, as sold, was: "{myth.claim}" —
        ...
        <Body>
          ...
          <Gold href={`/myth/${myth.id}/receipts`}>receipts page</Gold> — the verdict, and the named source: {myth.source}.
        ...
        <Body>
          ...
          <Gold href={`/myth/${myth.id}/instead`}>what actually works instead</Gold> — the real problem deserves a real
  ...
  return (
    <DeepFrame
      crumb={<><Gold href="/myths">The Myth Museum</Gold>{" · "}<Gold href={`/myth/${myth.id}`}>{myth.name}</Gold>{" · "}{SUB_LABELS[sub]}</>}
      h1={H1S[sub](myth.name)}
      videoLabel={`${myth.name} — ${SUB_LABELS[sub]}`}
    >
      <SiblingNav base={`/myth/${myth.id}`} subs={MYTH_SUBPAGES} current={sub} labels={SUB_LABELS} />
      {sections[sub]}
