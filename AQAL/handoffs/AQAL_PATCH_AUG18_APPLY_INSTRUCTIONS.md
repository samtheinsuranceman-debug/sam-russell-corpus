...
```
backup the DB  →  pnpm install (REQUIRED since 18P: drizzle-orm upgraded to 0.45.2 for a HIGH SQL-injection advisory, plus security overrides for mermaid/dompurify/body-parser — package.json and pnpm-lock.yaml both changed)  →  pnpm db:push  →  pnpm build
```
...
6h. New in 18Q: the "film briefing" video slot appears on EVERY page
