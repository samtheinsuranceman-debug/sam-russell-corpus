...
export const INTEGRATIONS: Integration[] = [
  ...
  // ── Voice, transcription, video, embeddings, vectors, gateway (added 23 Sep 2026) ──
  { id: "cartesia", name: "Cartesia", category: "voice", mode: "runtime", purpose: "Text to speech: a selectable site voice and the automatic fallback when ElevenLabs or HeyGen fails", envKeys: ["CARTESIA_API_KEY"], optionalKeys: ["CARTESIA_VOICE_ID", "CARTESIA_MODEL", "VOICE_PROVIDER"], docs: "https://docs.cartesia.ai/api-reference/tts/bytes", wiredTo: "speech.synthesize fallback, Voice Studio (voice router)" },
  { id: "deepgram", name: "Deepgram", category: "voice", mode: "runtime", purpose: "Speech to text (nova-3) for recorded voice sessions", envKeys: ["DEEPGRAM_API_KEY"], optionalKeys: ["TRANSCRIPTION_PROVIDER"], wiredTo: "POST /api/voice/transcribe (Thomas Goldman long voice sessions)" },
  { id: "assemblyai", name: "AssemblyAI", category: "voice", mode: "runtime", purpose: "Speech to text, fallback to Deepgram (or first, with TRANSCRIPTION_PROVIDER=assemblyai)", envKeys: ["ASSEMBLYAI_API_KEY"], optionalKeys: ["TRANSCRIPTION_PROVIDER"], docs: "https://www.assemblyai.com/docs/api-reference/transcripts/submit", wiredTo: "POST /api/voice/transcribe" },
  { id: "runway", name: "Runway", category: "voice", mode: "runtime", purpose: "Generative video from a prompt or a still image (RUNWAY_API_KEY also accepted)", envKeys: ["RUNWAYML_API_SECRET"], optionalKeys: ["RUNWAY_API_KEY"], docs: "https://docs.dev.runwayml.com/", wiredTo: "videoGen router (owner and admins)" },
  { id: "luma", name: "Luma AI", category: "voice", mode: "runtime", purpose: "Generative video (Ray), alternative to Runway (LUMA_API_KEY also accepted)", envKeys: ["LUMAAI_API_KEY"], optionalKeys: ["LUMA_API_KEY"], wiredTo: "videoGen router (owner and admins)" },
  { id: "voyage", name: "Voyage AI", category: "ai", mode: "runtime", purpose: "Text embeddings (voyage-4) for semantic search", envKeys: ["VOYAGE_API_KEY"], optionalKeys: ["VOYAGE_MODEL"], docs: "https://docs.voyageai.com/docs/embeddings", wiredTo: "server/voyage.ts client only; no feature embeds text yet" },
  { id: "pinecone", name: "Pinecone", category: "data", mode: "runtime", purpose: "Vector store for semantic search", envKeys: ["PINECONE_API_KEY"], optionalKeys: ["PINECONE_INDEX_HOST"], wiredTo: "server/pinecone.ts client only; no feature stores vectors yet" },
  { id: "portkey", name: "Portkey AI Gateway", category: "ai", mode: "runtime", purpose: "Gateway brain: one key to the providers in the Portkey Model Catalog", envKeys: ["PORTKEY_API_KEY"], optionalKeys: ["PORTKEY_MODEL"], docs: "https://portkey.ai/docs/product/model-catalog", wiredTo: "Brain Hub provider (providerRegistry)" },
];
