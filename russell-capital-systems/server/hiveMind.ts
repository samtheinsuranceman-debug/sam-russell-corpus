...
export interface HiveMember {
  ...
  model?: string;
  /** Council domains the owner marked this member strong in (owner panel). Empty = generalist. */
  domains?: string[];
  complete: (system: string, user: string) => Promise<string>;
}

/**
 * Order members for a domain: those the owner marked strong in it first (vault order preserved),
 * then generalists, then members marked for other domains. Stable, so the env roster order holds within a tier.
 */
export function rankMembersForDomain(members: HiveMember[], domain: string): HiveMember[] {
  const tier = (m: HiveMember) => (!m.domains || m.domains.length === 0 ? 1 : m.domains.includes(domain) ? 0 : 2);
  return members.map((m, i) => ({ m, i, t: tier(m) })).sort((a, b) => a.t - b.t || a.i - b.i).map((x) => x.m);
}
...
export async function askHive(userId: number, input: HiveAskInput, deps: HiveDeps = {}): Promise<HiveAnswer> {
  ...
  const members = rankMembersForDomain(await d.members(), domain);
