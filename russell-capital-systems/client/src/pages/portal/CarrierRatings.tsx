    ...
    <AppShell title="Carrier Ratings" subtitle="Source-labeled financial strength and index-crediting context">
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="inline-flex rounded-xl border border-emerald-400/15 bg-slate-950/40 p-1">
            <button className={`rounded-lg px-4 py-2 text-sm font-medium transition ${tab === "ratings" ? "bg-emerald-600 text-white" : "text-slate-400 hover:text-white"}`} onClick={() => setTab("ratings")}>Carrier ratings</button>
            <button className={`rounded-lg px-4 py-2 text-sm font-medium transition ${tab === "indices" ? "bg-emerald-600 text-white" : "text-slate-400 hover:text-white"}`} onClick={() => setTab("indices")}>Market Index Performance</button>
          </div>
        ...
        {tab === "ratings" ? (
          <section className="space-y-4">
            {carriersQuery.isLoading && <div className="flex items-center gap-2 rounded-2xl border border-emerald-400/15 bg-slate-950/40 p-5 text-sm text-emerald-100"><RefreshCw className="h-4 w-4 animate-spin" />Loading carrier ratings…</div>}
            ...
            <div className="grid gap-4 xl:grid-cols-2">
              {filteredCarriers.map(carrier => <article key={carrier.carrierId} className="rounded-2xl border border-emerald-400/15 bg-slate-950/45 p-5 shadow-lg shadow-emerald-950/10">
          ...
          <section className="space-y-4">
            ...
            {indexQuery.isLoading && <div className="flex items-center gap-2 rounded-2xl border border-emerald-400/15 bg-slate-950/40 p-5 text-sm text-emerald-100"><RefreshCw className="h-4 w-4 animate-spin" />Loading index performance…</div>}
