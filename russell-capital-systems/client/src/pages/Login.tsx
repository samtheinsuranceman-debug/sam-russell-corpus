    ...
    <main className="min-h-screen grid lg:grid-cols-[1.05fr_.95fr] bg-[#04100c] text-white">
      <section className="relative hidden overflow-hidden border-r border-emerald-400/15 lg:block">
        ...
        <div className="relative z-10 flex h-full flex-col justify-between p-12">
          ...
          <div className="max-w-xl">
            ...
            <p className="mt-5 max-w-lg text-lg leading-8 text-emerald-100/60">Server-enforced sessions protect client records, saved plans, scenarios, advisor tools, and the lead inbox. No built-in passwords, no hidden bypasses.</p>
          </div>
      ...
      <section className="grid place-items-center px-6 py-16">
        <div className="w-full max-w-md rounded-3xl border border-emerald-400/20 bg-white/[0.045] p-8 shadow-[0_30px_100px_rgba(6,95,70,.28)] backdrop-blur-xl">
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-emerald-300">Advisor portal</p>
          ...
          {mode?.ownerLogin && (
            <form onSubmit={submitOwnerLogin} className="mt-5" aria-label="Owner sign-in">
              <p className="leading-7 text-emerald-100/60">Owner sign-in for this installation.</p>
              ...
              <label className="mt-3 block text-sm text-emerald-100/70">Password
              ...
              <button type="submit" disabled={busy}
                className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-500 px-5 py-3 font-semibold text-white shadow-lg shadow-emerald-950/40 transition duration-200 hover:bg-emerald-400 active:scale-[0.97] disabled:opacity-60">
          ...
          {mode?.managedOAuth && (
            <>
              <p className="mt-3 leading-7 text-emerald-100/60">{mode.ownerLogin ? "Or continue with your authorized account." : "Continue with your authorized account."}</p>
              <button type="button" onClick={() => startLogin(returnPath)}
                className={`mt-5 flex w-full items-center justify-center gap-2 rounded-xl px-5 py-3 font-semibold transition duration-200 active:scale-[0.97] ${mode.ownerLogin ? "border border-emerald-300/30 text-emerald-100 hover:bg-emerald-500/15" : "bg-emerald-500 text-white shadow-lg shadow-emerald-950/40 hover:bg-emerald-400"}`}>
          ...
          {mode && !mode.ownerLogin && !mode.managedOAuth && (
            <p className="mt-3 leading-7 text-emerald-100/60">
              Sign-in is not configured on this host yet. The site owner sets <code className="text-emerald-200">OWNER_EMAIL</code> and <code className="text-emerald-200">OWNER_PASSWORD_HASH</code> in the server environment (see LAUNCH.md, section 4).
            </p>
          )}
