// ─── Navigation tree ────────────────────────────────────────────────────────
//
// PORTED VERBATIM from russell-capital-app @ ef74f3f. Kept byte-identical below
// the header so it can be re-synced from the donor with a plain copy; do not
// hand-edit the tree here to fix a broken target.
//
// IMPORTANT — this tree was authored against that repository's 593 routes, and
// this one serves 330. 124 of its 271 targets do not exist here. That is not a
// defect in the tree; it is the gap between the two repositories.
//
// So nothing renders this file directly. `lib/navTreeResolver.ts` validates
// every target against `shared/routeManifest.ts` and turns anything that does
// not resolve into a placeholder, which this file's own NavNode type already
// supports. A target that is not a real route therefore cannot become a dead
// menu link — it renders as a non-clickable label instead.
//
// `server/navTree-route-targets.test.ts` asserts that invariant, and pins the
// unresolved count so it can fall as routes arrive but never silently rise.
// ─────────────────────────────────────────────────────────────────────────────
        ...
        children: [
          {
            ...
            children: [
              l("Patient Presentation",  "/portal/client-presentation"),
              l("Patient Report Gen",    "/portal/client-report-gen"),
