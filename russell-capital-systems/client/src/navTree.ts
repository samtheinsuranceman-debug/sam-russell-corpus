// ─────────────────────────────────────────────────────────────────────────────
// Ported from russell-capital-app @ ef74f3f (PR-2b). React-free by design: the
// renderer resolves icons, so this file stays pure data and is testable in a
// node environment.
//
// EVERY LEAF WAS VALIDATED AGAINST shared/routeManifest.ts AT PORT TIME.
// The donor tree was authored against that repo's 612 routes; this build has
// 330. Of 271 distinct targets, 147 resolve here and 124 did not — those were
// converted to `isPlaceholder` rather than shipped as dead links, which is what
// the `isPlaceholder` field already existed for.
//
// A placeholder renders as non-interactive text. It is a promise, not a link:
// when a later migration lands the route, flip `ph("X")` back to
// `l("X", "/portal/x")`. `server/navTree.test.ts` fails if any non-placeholder
// leaf points at a path outside the manifest, so this can never silently rot.
// ─────────────────────────────────────────────────────────────────────────────
        ...
        children: [
          {
            ...
            children: [
              // Cross-listed: the same tool also appears under Mortgage Killer's
              // own branch. Intentional in the donor tree — one route, two ways
              // in. It creates no duplicate route.
              l("Mortgage Killer", "/portal/mortgage-killer"),
            ],
          },
        ],
