...
//
// Each NavNode is either a folder (has children) or a leaf (has `path` or
// `isPlaceholder`). Folders nest arbitrarily deep — the deepest branch here is
// five levels — which is the property a flat two-level sidebar cannot express.
//
// ## Ported from russell-capital-app, 2026-09-20
//
// The source tree carried 272 leaves. **124 of them pointed at pages that do
// not exist in this repository** — routes that only ever existed in
// `russell-capital-app`. Importing them verbatim would have produced 124 menu
// links that 404.
//
// Each of those is instead a `ph(...)` placeholder: the label and its position
// in the taxonomy survive, but it renders as un-clickable text rather than a
// dead link. This is the tree's own mechanism — 17 placeholders shipped in the
// original — so nothing was invented to hold them. When a page lands, its
// placeholder becomes `l(label, path)` and nothing around it moves.
//
//     272  source leaves
...
//
// `server/navTree.test.ts` and `server/navTreeRouteTargets.test.ts` enforce
// both invariants permanently: no live leaf may point outside the manifest,
// and no path may appear twice.
//
// This module is deliberately free of React and lucide imports so a
// node-environment test can import it directly. Icon tags are strings; the
// renderer maps them to components.
