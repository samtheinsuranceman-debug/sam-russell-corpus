...
const colorMap: Record<string, string> = {
  ...
  // Donor tinted this violet. This repo bans purple/violet anywhere in
  // client/src (server/concept16Homepage.test.ts asserts it), so the "purple"
  // key stays — category metadata still names it — but renders indigo.
...
/**
 * Relationship targets that this application actually serves.
 *
 * CALCULATOR_RELATIONSHIPS was authored against the donor's 612-route build and
 * names 95 distinct calculator slugs. This application serves 330 routes and
 * uses different slugs for several of the same tools (`medicare-irmaa` here vs
 * `medicare-irmaa-calc` there). Rendering the raw list would put dead links on
 * every calculator page.
 *
 * So every target is filtered through ROUTE_MANIFEST — the same list the smoke
 * tests hold App.tsx to. Links appear only for routes that resolve, and each
 * newly ported calculator lights up its own cross-links with no edit here.
 */
const SERVED = new Set<string>(ROUTE_MANIFEST);
export function servedRelated(slug: string): string[] {
  return (CALCULATOR_RELATIONSHIPS[slug] || []).filter((s) => SERVED.has(`/portal/${s}`));
}

export function RelatedCalculators({ currentRoute }: { currentRoute: string }) {
  const slug = currentRoute.replace("/portal/", "");
  const related = servedRelated(slug);
  ...
  return (
    <div className="mt-8 pt-6 border-t border-[#1a3055]">
      ...
      <div className="mt-3 text-center">
        {/* Donor pointed at /portal/calculator-hub, which this application does
            not serve. /calculators is the equivalent front door here. */}
