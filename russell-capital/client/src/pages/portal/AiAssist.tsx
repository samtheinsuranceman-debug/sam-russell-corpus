...
import { Brain, Search, Zap, BookOpen, ChevronRight, Library, Save, Check, ExternalLink, TrendingUp, BarChart3, Target, Info, Activity, Shield, Users, Circle, Settings, Calendar, RefreshCw, Plus, FileText, Download, Upload, Eye, EyeOff, ArrowRight, ArrowLeft, ArrowUp, Star, Share2, Copy, XCircle, HelpCircle, Icon, PieChartIcon} from 'lucide-react';
...
export default function AiAssist() {
  const { user } = useAuth();
  const [tab, setTab] = useState<"strategy" | "knowledge" | "analytics" | "reports" | "settings" | "unified">("strategy");
  const { state: brainState, getAllRecommendations, getFinancialHealth, getUnifiedSnapshot } = useAIBrain();
  const { entries: busEntries, getTotalDataPoints, getRecentEntries } = useUnifiedDataBus();
  const unifiedRecs = getAllRecommendations();
  const unifiedHealth = getFinancialHealth();
  const unifiedSnapshot = getUnifiedSnapshot();
  const recentBusEntries = getRecentEntries(10);
      ...
      <div className="px-6 pb-8">
        ...
        <div className="flex flex-wrap gap-1 mb-6 bg-[#0b1628] rounded-xl p-1 w-fit border border-[#12233e]">
          {[
            ...
            { key: "unified", label: "Unified Intelligence", icon: Activity },
          ].map((t) => {
            const Icon = t.icon;
            return (
              <button
          ...
          <div className="space-y-5">
            ...
            <div className="rc-card">
              <div className="text-white font-semibold mb-4 flex items-center gap-2">
                <Activity size={16} className="text-[#22c55e]" /> Unified Intelligence Feed
              </div>
              <p className="text-[#7a95b8] text-sm mb-4">
                All 164 features (105 calculators + 27 SI engines + 32 AI Pro Suite) feed data here in real-time.
                The AI Advisor uses this unified data to generate holistic, cross-feature recommendations.
              </p>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                <div className="bg-[#0f1e35] p-4 rounded-lg border border-indigo-500/20">
                  <div className="text-[#7a95b8] text-xs">Connected Features</div>
                  <div className="text-2xl font-bold text-white">{busEntries.length}</div>
                  <div className="text-indigo-400 text-xs">of 164</div>
                </div>
                <div className="bg-[#0f1e35] p-4 rounded-lg border border-purple-500/20">
                  <div className="text-[#7a95b8] text-xs">Data Points</div>
                  <div className="text-2xl font-bold text-white">{getTotalDataPoints()}</div>
                  <div className="text-purple-400 text-xs">flowing</div>
                </div>
                <div className="bg-[#0f1e35] p-4 rounded-lg border border-cyan-500/20">
                  <div className="text-[#7a95b8] text-xs">AI Insights</div>
                  <div className="text-2xl font-bold text-white">{unifiedRecs.length}</div>
                  <div className="text-cyan-400 text-xs">{unifiedRecs.filter(r => r.priority === 'critical').length} critical</div>
                </div>
                <div className="bg-[#0f1e35] p-4 rounded-lg border border-emerald-500/20">
                  <div className="text-[#7a95b8] text-xs">Brain Confidence</div>
                  <div className="text-2xl font-bold text-white">{brainState.confidenceScore}%</div>
                  <div className="text-emerald-400 text-xs">{brainState.connectionStatus}</div>
                </div>
              </div>

              {/* Source Breakdown */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
                <div className="bg-[#0f1e35] p-3 rounded-lg border border-indigo-500/20">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="w-2 h-2 bg-indigo-500 rounded-full"></span>
                    <span className="text-white text-sm font-medium">Calculators</span>
                  </div>
                  <div className="text-indigo-400 font-bold">{unifiedSnapshot.calculators} / 105 active</div>
                </div>
                <div className="bg-[#0f1e35] p-3 rounded-lg border border-purple-500/20">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="w-2 h-2 bg-purple-500 rounded-full"></span>
                    <span className="text-white text-sm font-medium">SI Engines</span>
                  </div>
                  <div className="text-purple-400 font-bold">{unifiedSnapshot.siEngines} / 27 active</div>
                </div>
                <div className="bg-[#0f1e35] p-3 rounded-lg border border-cyan-500/20">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="w-2 h-2 bg-cyan-500 rounded-full"></span>
                    <span className="text-white text-sm font-medium">AI Pro Suite</span>
                  </div>
                  <div className="text-cyan-400 font-bold">{unifiedSnapshot.aiProFeatures} / 32 active</div>
                </div>
              </div>

              {/* Health Score */}
              {unifiedHealth.score > 0 && (
                <div className="mb-6">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-white font-medium">Financial Health Score</span>
                    <span className={`text-lg font-bold ${unifiedHealth.score >= 80 ? 'text-emerald-400' : unifiedHealth.score >= 60 ? 'text-amber-400' : 'text-red-400'}`}>{unifiedHealth.score}/100</span>
                  </div>
                  <div className="w-full bg-gray-700 rounded-full h-2">
                    <div className={`h-2 rounded-full ${unifiedHealth.score >= 80 ? 'bg-emerald-500' : unifiedHealth.score >= 60 ? 'bg-amber-500' : 'bg-red-500'}`} style={{ width: `${unifiedHealth.score}%` }}></div>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mt-3">
                    {unifiedHealth.breakdown.map(cat => (
                      <div key={cat.category} className="bg-[#0f1e35] p-2 rounded text-xs">
                        <div className="text-[#7a95b8] capitalize">{cat.label}</div>
                        <div className={`font-bold ${cat.score >= 80 ? 'text-emerald-400' : cat.score >= 60 ? 'text-amber-400' : 'text-red-400'}`}>{cat.score}/100</div>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {/* AI Recommendations from Unified Data */}
            <div className="rc-card">
              <div className="text-white font-semibold mb-4">Cross-Feature AI Recommendations ({unifiedRecs.length})</div>
              {unifiedRecs.length === 0 ? (
                <div className="text-center py-6 text-[#7a95b8]">
                  <p>Run calculators, SI engines, or AI Pro features to generate cross-feature insights</p>
                </div>
              ) : (
                <div className="space-y-3 max-h-[400px] overflow-y-auto">
                  {unifiedRecs.slice(0, 15).map(rec => (
                    <div key={rec.id} className={`p-3 rounded-lg border ${
                      rec.priority === 'critical' ? 'border-red-500/30 bg-red-500/5' :
                      rec.priority === 'high' ? 'border-amber-500/30 bg-amber-500/5' :
                      rec.priority === 'medium' ? 'border-blue-500/30 bg-blue-500/5' :
                      'border-[#1e3a5f]/30 bg-gray-600/5'
                    }`}>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[#7a95b8] text-xs">{rec.category}</span>
                        <span className="text-gray-600 text-xs ml-auto">{rec.confidence}%</span>
                      </div>
                      <p className="text-white text-sm font-medium">{rec.title}</p>
                      <p className="text-[#7a95b8] text-xs mt-1">{rec.description}</p>
                      {rec.relatedTools.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-2">
                          {rec.relatedTools.map(tool => (
                            <span key={tool} className="px-1.5 py-0.5 bg-[#22c55e]/10 text-[#22c55e] rounded text-[9px]">{tool}</span>
                          ))}
                        </div>
                      )}
                    </div>
            ...
            {/* Recent Data Flow */}
            <div className="rc-card">
              <div className="text-white font-semibold mb-4">Recent Data Flow (Last 10 entries)</div>
              {recentBusEntries.length === 0 ? (
                <div className="text-center py-6 text-[#7a95b8]">
                  <p>No data flowing yet. Use any feature to see live data here.</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {recentBusEntries.map((entry, i) => (
                    <div key={`${entry.featureId}-${i}`} className="flex items-center justify-between p-3 bg-[#0f1e35] rounded-lg border border-[#12233e]">
                      <div className="flex items-center gap-3">
                        <span className="w-2 h-2 bg-green-500 rounded-full"></span>
                        <span className="text-white text-sm">{entry.featureName}</span>
                        <span className={`px-1.5 py-0.5 rounded text-[9px] border ${
                          entry.category === 'calculator' ? 'text-indigo-400 bg-indigo-500/10 border-indigo-500/20' :
                          entry.category === 'si-engine' ? 'text-purple-400 bg-purple-500/10 border-purple-500/20' :
                          'text-cyan-400 bg-cyan-500/10 border-cyan-500/20'
                        }`}>{entry.category}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-[#7a95b8] text-xs">{Object.keys(entry.payload).length} data points</span>
