...
## 9. Deployment round — the full team seated (September 16, 2026, first Railway deploy)

The owner asked for every connector's AI at the table. Seats and findings, with what was done:

| Seat | Model | Finding | Disposition |
|---|---|---|---|
| Security | Grok 4.3 | Email-link jti store is in-memory, so a link could be replayed across a restart inside its 15-minute window; token in URL can reach Referer/logs; log nothing from the auth path but hashed IP and status codes. | Replay window accepted for preview (a restart inside 15 minutes plus a captured link is required); moved to a DB-backed one-time record before production. Auth paths log only status and a truncated provider error. `returnPath` already refuses `//`, backslashes and anything not starting with `/`. |
| Deployment | GPT-5 | Pre-deploy may run before MySQL is reachable; the MySQL user may lack CREATE DATABASE; healthcheck must not race migrations; OpenAI's chat-completions endpoint is compatible with the forge-style gateway when tool calls map to `tools`/`tool_calls`. | Migration runner now retries the server connection for two minutes with backoff. Railway runs pre-deploy before the health check (Perplexity, Railway docs). CREATE DATABASE succeeded on the first deploy. |
| Consumer clarity | Gemini 2.5 Pro | "The only financial plan that reads your mental state first" is an unsubstantiated claim and "mental state" overstates what self-ratings are; "Decision capacity" implies a clinical or legal capacity assessment. | Public-edition copy rewritten to Gemini's wording: hero "Financial education that starts with how you feel", "Check-in score", "Your reported stress / stability / recent change", guardrail rows renamed (Emergency fund target, Borrowing limit, Max transaction size, Simple products, Automated actions, Second opinion, Review cadence), home card and pause text rewritten. Clinical-edition copy unchanged. |
| Engine robustness | DeepSeek V3.1 | Clamping and neutral fallbacks can hide a broken input behind a plausible number; add metamorphic properties (scale invariance, order independence of merged snapshots, idempotent sanitization); would not ship the empty-series throw. | The fallbacks are labelled in each profile's `assumptions`. Metamorphic properties queued for the next harness run. The empty-series precondition stays: an empty series has no baseline to degrade to, and every caller already guards it. |
| Invariant coverage | Claude Opus 4.1 (second Claude) | Add concurrency tests on the single-use link and rate-limit maps, HEAD/GET parity, redirects never carrying tokens, JSON depth bombs, batch-size limits; "no PHI in logs" was proven only with an input canary. | Queued as the next harness additions; the tRPC batch limit and JSON depth bomb are the first two. |
| Facts | Perplexity | Railway reference variables resolve into declared Docker ARGs and the runtime env; MySQL private domain on 3306 needs no IPv6 setting for mysql2; pre-deploy runs before the health check and a non-zero exit fails the deploy; Resend needs a verified sender domain; `trust proxy` makes `req.protocol` follow `X-Forwarded-Proto`. | Confirms the deployment design. The Resend sender domain must be verified for sign-in mail to reach anyone but the account owner. |
| Live verification | Perplexity Computer | First pass: Railway edge answered "Application not found" (deployment not yet routable). | Second pass scheduled once the deploy is green. |

First deploy result: build succeeded on Railway (the `.npmrc` fix held), the database was created, migrations 0000–0010 applied, and 0011 failed because the foundation's `breach_incidents` table, along with six others (`audit_logs`, `client_leads`, `deletion_requests`, `mental_credit_scores`, `personality_profiles`, `personality_responses`), exists in `drizzle/schema.ts` but in no migration. A generated migration that creates the seven tables now runs before 0011.
