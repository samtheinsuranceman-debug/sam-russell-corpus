...
export default function AdvisorIncomeCalculator() {
  const [clientAge, setClientAge] = useState<number>(60);
      ...
      <div className="container py-6 space-y-6" id="advisor-income-calculator">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          <Card className="bg-slate-900 border-slate-800">
            <CardContent className="p-4 space-y-2">
              <Label className="text-slate-300">Client Age</Label>
          ...
          <Card className="bg-slate-900 border-slate-800">
            <CardContent className="p-4 space-y-2">
              <Label className="text-slate-300">Spouse Age (Joint)</Label>
              <div className="flex items-center gap-2">
                <Users className="w-4 h-4 text-slate-500" />
                <NumberInput
                  value={spouseAge}
          ...
          <Card className="bg-slate-900 border-slate-800">
            <CardContent className="p-4 space-y-2">
              <Label className="text-slate-300">Income Start Age</Label>
              <div className="flex items-center gap-2">
          ...
          <Card className="bg-slate-900 border-slate-800">
            <CardContent className="p-4 space-y-2">
              <Label className="text-slate-300">Life Expectancy</Label>
              <div className="flex items-center gap-2">
        ...
        {viewMode === "advanced" && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-slate-900 rounded-lg border border-slate-800">
            ...
            <div className="space-y-2">
              <Label className="text-slate-300">Sort Results By</Label>
            ...
            <CardContent className="p-6">
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
                <div>
                  ...
                  <p className="text-slate-400">Based on your {deferralYears}-year deferral strategy</p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-700">
                  <div className="text-sm text-slate-400 mb-1">Carrier & Product</div>
        ...
        <Tabs defaultValue="ranking" className="space-y-4">
          <TabsList className="flex flex-wrap gap-1 h-auto p-1 bg-slate-900 border border-slate-800 rounded-lg">
            <TabsTrigger
              value="ranking"
              className="text-xs sm:text-sm data-[state=active]:bg-slate-700 data-[state=active]:text-white text-slate-400"
            >
                    ...
                    <tbody>
                      {results.map((r, i) => (
                        <tr
                          ...
                          className={`border-b border-slate-800 hover:bg-slate-800/50 ${i === 0 ? "bg-green-500/10 font-semibold" : ""} ${i < 3 ? "border-l-4 border-l-green-500" : ""}`}
                        >
                          ...
                          <td className="p-2 text-xs whitespace-nowrap text-blue-100">
                            {r.product}<br />
                            <span className="text-slate-400">{r.rider}</span>
                          </td>
                    ...
                    <tbody>
                      {accumulation.map((row) => {
                        ...
                        return (
                          <tr
                            key={row.year}
                            className={`border-b border-slate-800 hover:bg-slate-800/50 ${isSelected ? "bg-green-500/10 font-bold" : ""}`}
                          >
                            <td className="p-2 text-slate-300">Year {row.year}</td>
                            <td className="p-2 text-white">Age {row.age}</td>
                            <td className="p-2 text-right font-mono text-slate-300">{fmt(row.startingBase)}</td>
                            ...
                            <td className="p-2 text-center text-slate-300">{fmtPct(row.withdrawalRate)}</td>
                  ...
                  <table className="w-full text-sm border-collapse">
                      ...
                      <tr className="border-b-2 border-blue-500">
                        ...
                        {[0, 1, 2, 3, 5, 7, 10, 15].filter((yr) => clientAge + yr <= 90).map((yr) => (
                          <th key={yr} className="text-right p-2 font-semibold whitespace-nowrap text-white">
                            Age {clientAge + yr}
                            <div className="text-xs font-normal text-slate-400">Yr {yr}</div>
                          </th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {CARRIER_DATABASE.slice(0, 5).map((cp, ci) => (
                        <tr key={ci} className="border-b border-slate-800 hover:bg-slate-800/50">
                          <td className="p-2 font-medium whitespace-nowrap text-blue-100">
                            {cp.carrier}
                            <div className="text-xs text-slate-400">{cp.rollupRate}% rollup</div>
                          </td>
                          {[0, 1, 2, 3, 5, 7, 10, 15].filter((yr) => clientAge + yr <= 90).map((yr) => {
                            ...
                            return (
                              ...
                              >
                                {fmt(income)}
                                <div className="text-[10px] text-slate-400">{fmt(Math.round(income / 12))}/mo</div>
                              </td>
                            );
                          })}
                        </tr>
                ...
                <div className="space-y-3">
                  {results.slice(0, 10).map((r, i) => {
                    ...
                    return (
                      <div
                        key={`${r.carrier}-${r.product}`}
                        className={`rounded-lg border border-slate-800 p-4 ${i === 0 ? "border-green-500/50 bg-green-500/10" : "bg-slate-900"}`}
                      >
                        <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                          ...
                          <div className="flex items-center gap-3 text-sm">
                            ...
                            <span className="text-slate-400">({fmt(r.monthlyIncome)}/mo)</span>
                          </div>
                        </div>
