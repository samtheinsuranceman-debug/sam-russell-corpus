  ...
  return (
    <AppShell title="Planning Cases" subtitle="Persistent client planning workflows">
      <div className="mx-auto max-w-[1500px] space-y-6 p-4 md:p-7">
        <section className="overflow-hidden rounded-3xl border border-emerald-400/20 bg-gradient-to-br from-emerald-950/80 via-slate-950/90 to-emerald-950/55 p-6 shadow-2xl shadow-emerald-950/40">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <Button onClick={() => setShowCreate(value => !value)} className="bg-emerald-500 text-white hover:bg-emerald-400">
        ...
        {showCreate && (
          <section className="rounded-2xl border border-emerald-400/20 bg-slate-950/75 p-5 shadow-xl">
            <div className="grid gap-4 md:grid-cols-[1fr_1fr_auto] md:items-end">
              ...
              <label className="space-y-2 text-sm text-slate-300">
                ...
                <select value={newClientId} onChange={event => setNewClientId(event.target.value)} className="h-10 w-full rounded-md border border-emerald-400/20 bg-slate-950 px-3 text-sm text-white">
        ...
        <div className="grid gap-6 xl:grid-cols-[340px_minmax(0,1fr)]">
          <aside className="rounded-2xl border border-emerald-400/15 bg-slate-950/70 p-3 shadow-xl">
            ...
            {casesQuery.isLoading ? (
              <div className="flex items-center justify-center py-16 text-emerald-300"><Loader2 className="h-6 w-6 animate-spin" /></div>
            ) : casesQuery.isError ? (
              ...
              <div className="m-2 rounded-xl border border-dashed border-emerald-400/25 p-6 text-center">
                <ClipboardList className="mx-auto mb-3 h-8 w-8 text-emerald-300" />
              ...
              <div className="space-y-2">
                {cases.map(item => (
                  <button
                    ...
                    className={`w-full rounded-xl border p-4 text-left transition ${selectedId === item.id ? "border-emerald-400/45 bg-emerald-500/15" : "border-transparent bg-white/[0.03] hover:border-emerald-400/20 hover:bg-emerald-500/10"}`}
                  >
                    <div className="flex items-start justify-between gap-3">
                      <p className="line-clamp-2 text-sm font-semibold text-white">{item.title}</p>
                      {item.status === "archived" && <Archive className="h-4 w-4 shrink-0 text-slate-500" />}
                    </div>
                    <p className="mt-2 text-xs text-emerald-200">{clientNameById.get(item.clientId ?? -1) ?? "Unlinked case"}</p>
          ...
          <main className="min-w-0">
            {!selectedId ? (
              <div className="flex min-h-[520px] items-center justify-center rounded-2xl border border-dashed border-emerald-400/25 bg-slate-950/50 p-10 text-center">
                <div><Users className="mx-auto mb-4 h-10 w-10 text-emerald-300" /><p className="font-medium text-white">Select or create a planning case</p></div>
              </div>
            ) : selectedCase.isLoading ? (
              <div className="flex min-h-[520px] items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-emerald-300" /></div>
              ...
              <div className="space-y-5">
                <section className="rounded-2xl border border-emerald-400/15 bg-slate-950/70 p-5 shadow-xl">
                  <div className="grid gap-4 lg:grid-cols-[1fr_180px_180px_auto] lg:items-end">
                    ...
                    <label className="space-y-2 text-sm text-slate-300"><span>Status</span><select value={status} onChange={event => setStatus(event.target.value as keyof typeof STATUS_LABELS)} className="h-10 w-full rounded-md border border-emerald-400/20 bg-slate-950 px-3 text-sm text-white">{Object.entries(STATUS_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
                    <label className="space-y-2 text-sm text-slate-300"><span>Current stage</span><select value={stage} onChange={event => setStage(event.target.value as (typeof WORKFLOW_STAGES)[number])} className="h-10 w-full rounded-md border border-emerald-400/20 bg-slate-950 px-3 text-sm capitalize text-white">{WORKFLOW_STAGES.map(value => <option key={value} value={value}>{value}</option>)}</select></label>
                  ...
                  <div className="mt-5 grid grid-cols-3 gap-2 sm:grid-cols-6">
                    {WORKFLOW_STAGES.map((value, index) => {
                      ...
                      return <button key={value} onClick={() => setStage(value)} className={`rounded-xl border px-2 py-3 text-center text-[11px] font-medium capitalize ${active ? "border-emerald-300/60 bg-emerald-500/25 text-white" : complete ? "border-emerald-400/20 bg-emerald-500/10 text-emerald-200" : "border-white/10 bg-white/[0.02] text-slate-500"}`}>{complete && <CheckCircle2 className="mx-auto mb-1 h-4 w-4" />}{value}</button>;
                    })}
                  </div>
                </section>
                ...
                <section className="rounded-2xl border border-emerald-400/15 bg-slate-950/70 p-5 shadow-xl">
                  <div className="flex items-center justify-between gap-4"><div><h2 className="font-semibold text-white">Case notes</h2><p className="mt-1 text-xs text-slate-400">Advisor, client, compliance, and system notes are timestamped and persisted.</p></div><MessageSquarePlus className="h-5 w-5 text-emerald-300" /></div>
                  ...
                  <div className="mt-5 space-y-3">
                    {notesQuery.isLoading ? <Loader2 className="h-5 w-5 animate-spin text-emerald-300" /> : (notesQuery.data ?? []).length === 0 ? <p className="rounded-xl border border-dashed border-white/10 p-5 text-sm text-slate-500">No notes have been added.</p> : (notesQuery.data ?? []).map(item => <article key={item.id} className="rounded-xl border border-white/10 bg-white/[0.03] p-4"><div className="flex items-center justify-between gap-3"><span className="text-xs font-semibold uppercase tracking-wider text-emerald-300">{item.noteType}</span><span className="text-xs text-slate-500">{formatDate(item.createdAt)}</span></div><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-200">{item.content}</p></article>)}
                  </div>
                </section>
              </div>
            )}
