...
export default function TapeRecorderAdvisor({ onJourney }: { onJourney?: (j: JourneyView) => void }) {
  ...
  // `via` is the provider that answered; `fallback` is set when the site's
  // chosen voice was unavailable and another one spoke in its place.
  const [speaker, setSpeaker] = useState<{ via: string; fallback: string | null } | null>(null);
  ...
  const speak = useCallback(async (text: string) => {
    ...
    try {
      ...
      if (r.ok) {
        setSpeaker({ via: r.via, fallback: r.fallback ?? null });
      ...
      <div className="relative overflow-hidden rounded-[28px] border border-emerald-400/25 bg-[linear-gradient(160deg,#1a1c26,#0c0e15_60%,#090b11)] p-5 shadow-[0_40px_120px_rgba(0,0,0,.6),inset_0_1px_0_rgba(255,255,255,.06)] sm:p-7">
        ...
        {mode === "speaking" && (
          <p className="mt-0.5 text-[10px] uppercase tracking-[0.22em] text-slate-500">
            {speaker
              ? speaker.fallback
                ? `Voice · ${speaker.via} — fell back to ${speaker.fallback}, not the site's chosen voice`
                : `Voice · the site's voice, via ${speaker.via}`
              : "Voice · device speech (the site's voice is not configured)"}
          </p>
        )}
