...
import { federalTaxOnTaxable, FEDERAL_RATES_SOURCE } from "@shared/taxBracketEngine";

/* ─── Federal tax: current-year single-filer table from shared/taxRules.ts ─── */
function calcFederalTax(taxableIncome: number): number {
  return federalTaxOnTaxable(taxableIncome, "single");
}
        ...
        <CardContent className="pt-4">
          ...
          {enabled && (
            <div className="space-y-4 pt-4 border-t border-orange-500/20">
              ...
              <p className="text-[10px] text-muted-foreground">Federal tax source: {FEDERAL_RATES_SOURCE.short}, single-filer table.</p>
            </div>
          )}
        </CardContent>
      </Card>
